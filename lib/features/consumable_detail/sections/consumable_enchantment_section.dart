...
class ConsumableEnchantmentSection extends StatelessWidget {
  final ItemVO item;
  const ConsumableEnchantmentSection({super.key, required this.item});
...
class EnchantmentRow extends StatelessWidget {
  ...
  Column _buildEnchantmentComplete(BuildContext context, ConsumableDetailProvider provider) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Padding(
    ...
    return GestureDetector(
      onTap: () {
        ...
        if (context.read<CraftingProvider>().craftingEnchantmentList.isNotEmpty) {
          context.read<CraftingProvider>().getCraftingMarketPrice(enchantmentId: enchantItem.enchantment);
        }
      },
      child: Padding(
        ...
        child: Stack(
          children: [
            ...
            Visibility(
              visible:
                  context.read<ConsumableDetailProvider>().selectedEnchantment.enchantment == enchantItem.enchantment,
              child: Positioned(
                bottom: 12,
                right: 11,
