    ...
    return Consumer<ItemDetailProvider>(builder: (context, provider, child) {
      return Column(
        children: [
          ...
          provider.enchanmentList.isNotEmpty
              ? GearStatTable()
              : SizedBox(),
        ],
      );
            ...
            itemBuilder: (context, index) {
              return GearStatRow(
                firstStat: StatVO(
                    ...
                    value:
                        provider.selectedQuality.stats.toDoubleStatList()[index].firstStat.value),
                secondStat: StatVO(
                    name: provider.selectedQuality.stats.toDoubleStatList()[index].secondStat.name,
                    value:
                        provider.selectedQuality.stats.toDoubleStatList()[index].secondStat.value),
              );
            });
      }
    );
                ...
                ? SizedBox(
                    ...
                    child: DropdownButtonFormField2(
                      ...
                      onChanged: (quality) {
                        if (quality != null) {
                          provider
                              .selectQuality(provider.selectedEnchantment.searchQualityVO(quality));
                        }
                      },
      ...
      child: Row(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              SizedBox(width: 100, child: InterText(firstStat.name, TextStyle(fontSize: TEXT_SMALL))),
              SizedBox(width: 20, child: InterText(":",TextStyle(fontSize: TEXT_SMALL))),
              SizedBox(width: 40, child: InterText(firstStat.value, TextStyle(fontSize: TEXT_SMALL), TextAlign.end))
            ],
          ),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              SizedBox(width: 100, child: InterText(secondStat.name,TextStyle(fontSize: TEXT_SMALL))),
              Visibility(
                visible: secondStat.name.isNotEmpty,
                child: SizedBox(width: 20, child: InterText(":",TextStyle(fontSize: TEXT_SMALL))),
              ),
              SizedBox(width: 40, child: InterText(secondStat.value, TextStyle(fontSize: TEXT_SMALL), TextAlign.end))
            ],
          ),
        ],
      ),
